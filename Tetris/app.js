document.addEventListener("DOMContentLoaded", () => {
  ...
  // Handle rotations at the edge of the screen
  function isAtRight() {
  ...
  function checkRotatedPosition(P) {
    ...
    if ((P + 1) % width < 4) {
      ...
      if (isAtRight()) {
        ...
        checkRotatedPosition(P); //check again then pass position from start, since long block might need to move more.
      }
    } else if (P % width > 5) {
      if (isAtLeft()) {
        currentPosition -= 1;
  ...
  // Mini Grid
  // mini grid to provide future Tetrominos falling
  const displaySquares = document.querySelectorAll(".mini-grid div");
  const displayWidth = 4;
  const displayIndex = 0;

  // Tetrominos displayed in mini grid
